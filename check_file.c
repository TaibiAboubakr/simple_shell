 ...
 *
 * @shellname: The name of the shell invoking the function
 * @filename: pointer to the file associated with the error
...
int check_file(char *shellname, char *filename, char **args, int count)
{
	...
	if (!((stat(filename, &f_stat) == 0) && (filename[0] == '.' || filename[0] == '/')))
	{
		print_err_file(shellname, filename, "not found", count);
		free(args);
		return (127);
	}
	/* if ((stat(filename, &f_stat) == 0)) */
	if (!(f_stat.st_mode & S_IXUSR))
	{
		print_err_file(shellname, filename, "permission denied", count);
		free(args);
		return (126);
	}
	return (0);
